    ...
    private async Task FetchMedia(string query)
    {
        ...
        try
        {
            ...
            _results = fetchedResults.Data
                    .Select(m => 
                    (IListItem)new ListItem(new OpenUrlCommand($"https://anilist.co/{(m.Type == MediaType.Anime ? "anime" : "manga")}/{m.Id}/{m.Title.PreferredTitle}"))
                    //: new ListItem(new MediaPreviewPage())
                    {
                        Title = m.Title.PreferredTitle,
                        Subtitle = $"{m.Format} • {m.Status} • {m.AverageScore}%",
                        Icon = new IconInfo(m.Cover.LargeImageUrl.AbsoluteUri),
                        Details = !AnilistExt.AppSettings.UseAdaptivePreview ? new Details()
                        {
                        ...
                        MoreCommands = [
                            ...
                            new CommandContextItem(new NoOpCommand()) { Title = "Add to List", Icon = new IconInfo("\uF8AA"),Command = new AddToListPage(m)},
                            new CommandContextItem(new NoOpCommand()) { Title = "Remove from List", Icon = new IconInfo("\uE738"), Command = new RemoveFromUserList(m.Id)},
